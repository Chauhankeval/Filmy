        ...
        <div className='flex justify-between flex-wrap mt-3 p-3 mx-3'>
            {loading ? (
                <div className='w-full flex justify-center items-center h-96'>
                    <InfinitySpin visible={true} width="200" color="white" ariaLabel="infinity-spin-loading" />
                </div>
            ) : (
                data.map((ele) => (
                    <Link key={ele.id} to={`/detail/${ele.id}`}>
                        <div className='card p-2 shadow-lg hover:-translate-y-2 cursor-pointer  font-medium mt-6 transition-all duration-500 rounded-sm'>
                            <img className='h-60 md:h-72' src={ele.image} alt={ele.title} />
